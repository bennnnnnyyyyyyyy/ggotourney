( function( $ ) {
    ...
    function maiko_triger_tabs_image($scope) {
        ...
        // Handle Style 2 - Clip Path Wipe
        const $items_style2 = $scope.find('.pxl-images-slip.style-2 .pxl-item--content');

        if ($items_style2.length > 0) {
            $items_style2.each(function(index, item) {
                if (index === 0) {
                    // First image: wipes out with clip-path
                    gsap.fromTo(item, {
                        opacity: 1,
                    ...
                    // Subsequent images: wipe in with clip-path - use same trigger timing
                    gsap.fromTo(item, {
                        opacity: 1,
